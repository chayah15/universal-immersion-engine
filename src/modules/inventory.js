...
export async function scanLootAndStatus() {
    ...
    const prompt = `
Analyze the last message to see if the player's inventory or status changed.
Message: "${text.slice(0, 1000)}"

Rules:
- Only detect explicit changes (e.g., "You found a sword", "You take 5 damage").
- Ignore metaphors.
- Return JSON only.
- DO NOT track currency/gold (handled separately).

JSON Format:
{
  "loot": [{"name": "Item Name", "type": "Item Type (Weapon/Potion/etc)", "desc": "Short desc"}],
  "lost": ["Item Name"],
  "hp": 0,
  "xp": 0
}
`;
    ...
    try {
        ...
        const data = JSON.parse(cleanOutput(res, "json"));
        let changed = false;
        ...
        if (Array.isArray(data.loot) && data.loot.length > 0) {
            ...
            for (const item of data.loot) {
                ...
                notify("success", `+ ${item.name}`, "Loot", "loot");
            }
            changed = true;
        }
        ...
        if (Array.isArray(data.lost) && data.lost.length > 0) {
            ensureModel(s);
            for (const name of data.lost) {
                const idx = s.inventory.items.findIndex(i => i.name.toLowerCase().includes(name.toLowerCase()));
                if (idx !== -1) {
                    s.inventory.items.splice(idx, 1);
                    notify("warning", `- ${name}`, "Loot", "loot");
                    changed = true;
                }
            }
        }
        ...
        if (data.hp && data.hp !== 0) {
            s.hp = Math.max(0, Math.min(s.maxHp || 100, (s.hp || 100) + data.hp));
            changed = true;
        }
        if (data.xp && data.xp > 0) {
            s.xp = (s.xp || 0) + data.xp;
            applyLevelingProgress(s);
            notify("success", `+ ${data.xp} XP`, "XP", "xp");
            changed = true;
        }
        ...
        if (changed) {
            saveSettings();
            updateLayout();
            import("./features/items.js").then(mod => { if(mod?.render) mod.render(); });
        }
    } catch (e) {
        console.warn("[UIE] Loot Scan Failed:", e);
    }
}
